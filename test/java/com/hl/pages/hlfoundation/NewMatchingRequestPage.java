...
public class NewMatchingRequestPage extends TestBase {

	WebDriver webDriver;
	LandingPage landingPage;
	String filePath = excel.getCellData("MatchingRequest1Files", "FilePath", 2);
	String fileName="";

	public NewMatchingRequestPage(WebDriver driver) {
		webDriver = driver;
		landingPage=new LandingPage(driver);
	}

	public By byCreateButton=By.xpath("//button[*[text()='Create Matching Request']]");
	public void clickCreateButton() {
		scrollUpDownToElement(webDriver, byCreateButton, "Create Button" );
		clickElement(byCreateButton, "Create Button");
		try {Thread.sleep(500);} catch (InterruptedException e) {e.printStackTrace();}
	}
	public void isCreateButtonEnable() {
		scrollUpDownToElement(webDriver, byCreateButton, "Create Button", -200);
		if(isElementEnabled(byCreateButton,"Create Button", ExceptionHandling.FAIL)) 
			TestUtilities.logPassWithScreenshot("Create Button is enabled");
		else
			TestUtilities.logFailedWithScreenshot("Create Button is NOT enabled");
	}
	public void isCreateButtonDisable() {
		scrollUpDownToElement(webDriver, byCreateButton, "Create Button", -200);
		if(isElementDisabled(byCreateButton,"Create Button", ExceptionHandling.FAIL)) 
			TestUtilities.logPassWithScreenshot("Create Button is disabled");
		else
			TestUtilities.logFailedWithScreenshot("Create Button is NOT disabled");
	}

	public By byClearButton=By.xpath("//button[*[text()='Clear Form']]");
	public void clickClearFormButton() {
		scrollUpDownToElement(webDriver, byClearButton, "Clear Button" ,-200);
		clickElement(byClearButton, "Clear Button");
		try {Thread.sleep(500);} catch (InterruptedException e) {e.printStackTrace();}
	}
	public void isClearFormButtonEnable() {
		scrollUpDownToElement(webDriver, byClearButton, "Clear Form Button", -200);
	...
	public void uploadMonetaryDoc(String fileType) {
		...
		switch(fileType) {
		case "ValidFile":fileName = excel.getCellData("MatchingRequest1Files", "MonetaryFileName", 2);break;
		case "NonPDFFile":fileName = excel.getCellData("MatchingRequest1Files", "NonPDF", 2);break;
		case "Over100MBFile":fileName = excel.getCellData("MatchingRequest1Files", "Over100MB", 2);break;
		}
	...
	public void uploadCompletedDoc(String fileType) {
		...
		switch(fileType) {
		case "ValidFile":fileName = excel.getCellData("MatchingRequest1Files", "CompletedFileName", 2);break;
		case "NonPDFFile":fileName = excel.getCellData("MatchingRequest1Files", "NonPDF", 2);break;
		case "Over100MBFile":fileName = excel.getCellData("MatchingRequest1Files", "Over100MB", 2);break;
		}
		try {TestUtilities.copyPaste(filePath+fileName);} catch (Exception e) {e.printStackTrace();}
	...
	public void uploadNonprofitDoc(String fileType) {
		...
		switch(fileType) {
		case "ValidFile":fileName = excel.getCellData("MatchingRequest1Files", "NonprofitFileName", 2);break;
		case "NonPDFFile":fileName = excel.getCellData("MatchingRequest1Files", "NonPDF", 2);break;
		case "Over100MBFile":fileName = excel.getCellData("MatchingRequest1Files", "Over100MB", 2);break;
		}
		try {TestUtilities.copyPaste(filePath+fileName);} catch (Exception e) {e.printStackTrace();}
	...
	public void ValidateTitle() {
		...
		if (isElementDisabled(byTitleEnabled,"Title", ExceptionHandling.FAIL)) {
			...
			if (executeSQLSelectTitle().equals(getTextBoxText(byTitleEnabled,"Title", ExceptionHandling.FAIL))) 
	...
	public void ValidateDonationAmount() {
		...
		if (isElementVisible(byDonationAmountValidationMessage,donationAmountValidationMessage, ExceptionHandling.FAIL)) {
			...
			clickClearFormButton();
			scrollUpDownToElement(webDriver, byDonationAmount, "Donation Amount", -200);
			if (isNotElementVisible(byDonationAmountValidationMessage,donationAmountValidationMessage, ExceptionHandling.FAIL)) {
				try {Thread.sleep(500);} catch (InterruptedException e) {e.printStackTrace();}
			} else
				TestUtilities.logFailedWithScreenshot("Donation Amount Validation message is NOT cleared after clicking ClearForm Button");
		} else 			
		...
		if (isElementVisible(byDonationAmountValidationMessage,donationAmountValidationMessage, ExceptionHandling.FAIL)) {
			...
			clickClearFormButton();
			scrollUpDownToElement(webDriver, byDonationAmount, "Donation Amount", -200);
			isNotElementVisible(byDonationAmountValidationMessage,donationAmountValidationMessage, ExceptionHandling.FAIL);
		}
		else
		...
		if (isElementVisible(byDonationAmountValidationMessage,donationAmountValidationMessage, ExceptionHandling.FAIL)) {
			...
			isNotElementVisible(byDonationAmountValidationMessage,donationAmountValidationMessage, ExceptionHandling.FAIL);
			if ("".equals(getDonationAmount())) 
				TestUtilities.logPassWithScreenshot("Donation Amount is cleared after clicking ClearForm Button");
			else
				TestUtilities.logPassWithScreenshot("Donation Amount is NOT cleared after clicking ClearForm Button");
		}
	}
	...
	public By byMatchingAmountValidationMessage=By.xpath("//*[contains(text(),'"+matchingAmountValidationMessage+"')]");
	...
	public void ValidateMatchingAmount() {
		...
		if (isElementVisible(byMatchingAmountValidationMessage,matchingAmountValidationMessage, ExceptionHandling.FAIL)) {
			...
			clickClearFormButton();
			scrollUpDownToElement(webDriver, byMatchingAmount, "Matching Amount", -200);
			if (isNotElementVisible(byMatchingAmountValidationMessage,matchingAmountValidationMessage, ExceptionHandling.FAIL)) { 
				try {Thread.sleep(500);} catch (InterruptedException e) {e.printStackTrace();}
				TestUtilities.logPassWithScreenshot("Matching Amount validation message is cleared after clicking ClearForm Button");
			} else
				TestUtilities.logFailedWithScreenshot("Matching Amount validation message is NOT cleared after clicking ClearForm Button");
		} else
		...
		if (isElementVisible(byMatchingAmountValidationMessage,matchingAmountValidationMessage, ExceptionHandling.FAIL)) {
			...
			clickClearFormButton();
			scrollUpDownToElement(webDriver, byMatchingAmount, "Matching Amount", -200);
			isNotElementVisible(byMatchingAmountValidationMessage,matchingAmountValidationMessage, ExceptionHandling.FAIL);
		} else
		...
		if (isElementVisible(byMatchingAmountValidationMessage,matchingAmountValidationMessage, ExceptionHandling.FAIL)) { 
			...
			isNotElementVisible(byMatchingAmountValidationMessage,matchingAmountValidationMessage, ExceptionHandling.FAIL);
			if ("".equals(getMatchingAmount())) 
				TestUtilities.logPassWithScreenshot("Matching Amount is cleared after clicking ClearForm Button");
			else
				TestUtilities.logFailedWithScreenshot("Matching Amount is NOT cleared after clicking ClearForm Button");
		}
	}

	public String donationDateValidationMessage="Date must be today's date or a previous date.";
	public By byDonationDateValidationMessage=By.xpath("//*[contains(text(),'Date must be today')]");
	public By byDonationDate=By.id("donationDateInputField");
	public void setDonationDate(String date) {
		setTextBoxText(byDonationDate,date,"Donation Date");
	}
	...
	public void ValidateDonationDate() {
		...
		if (isElementVisible(byDonationDateValidationMessage,donationDateValidationMessage, ExceptionHandling.FAIL)) { 
			TestUtilities.logPassWithScreenshot("Donation Date is required");
			clickClearFormButton();
			scrollUpDownToElement(webDriver, byDonationDate, "Donation Date", -200);
			if (isNotElementVisible(byDonationDateValidationMessage,donationDateValidationMessage, ExceptionHandling.FAIL)) {
				try {Thread.sleep(500);} catch (InterruptedException e) {e.printStackTrace();}
				TestUtilities.logPassWithScreenshot("Donation Date validation message is cleared after clicking ClearForm Button");
			}else
				TestUtilities.logFailedWithScreenshot("Donation Date validation message is NOT cleared after clicking ClearForm Button");
		}else
			TestUtilities.logFailedWithScreenshot("Donation Date is NOT required");
		setDonationDate("01012025");
		Actions act = new Actions(driver);
		act.sendKeys(Keys.TAB).build().perform();
		if (isElementVisible(byDonationDateValidationMessage,donationDateValidationMessage, ExceptionHandling.FAIL)) {
			TestUtilities.logPassWithScreenshot("Donation Date validation message is displayed for future date");
			clickClearFormButton();
			scrollUpDownToElement(webDriver, byDonationDate, "Donation Date", -200);
			isNotElementVisible(byDonationDateValidationMessage,donationDateValidationMessage, ExceptionHandling.FAIL);
		}else
			TestUtilities.logFailedWithScreenshot("Donation Date validation message is NOT displayed for future date");
		try {Thread.sleep(500);} catch (InterruptedException e) {e.printStackTrace();}
		setDonationDate("0101");
		act = new Actions(driver);
		act.sendKeys(Keys.TAB).build().perform();
		if (isElementVisible(byDonationDateValidationMessage,donationDateValidationMessage, ExceptionHandling.FAIL)) { 
			TestUtilities.logPassWithScreenshot("Donation Date validation message is displayed for invalid date");
			clickClearFormButton();
			scrollUpDownToElement(webDriver, byDonationDate, "Donation Date", -200);
			isNotElementVisible(byDonationDateValidationMessage,donationDateValidationMessage, ExceptionHandling.FAIL);
		}else
			TestUtilities.logFailedWithScreenshot("Donation Date validation message is NOT displayed for invalid date");
		setDonationDate("02022023");
		act = new Actions(driver);
		act.sendKeys(Keys.TAB).build().perform();
		if ("2023-02-02".equals(getDonationDate())) 
			TestUtilities.logPassWithScreenshot("Donation Date is accepted for today's date or a previous date");
		else
			TestUtilities.logFailedWithScreenshot("Donation Date is NOT accepted for today's date or a previous date");
		clickClearFormButton();
		scrollUpDownToElement(webDriver, byDonationDate, "Donation Date", -200);
		isNotElementVisible(byDonationDateValidationMessage,donationDateValidationMessage, ExceptionHandling.FAIL);
		if ("".equals(getDonationDate())) 
			TestUtilities.logPassWithScreenshot("Donation Date is cleared after clicking ClearForm Button");
		else
			TestUtilities.logFailedWithScreenshot("Donation Date is NOT cleared after clicking ClearForm Button");
	}
	...
	public void setPreviousCharityRadioPositive() {
		scrollUpDownToElement(webDriver, byPreviousCharityRadioNegative, "Previous Charity Yes Radio button",-300);
		selectRadioButton(byPreviousCharityRadioPositive,"Have you donated to this charity before? Yes");
	}

	public By byWorkedCharityDiv=By.xpath("//div[*[text()='If not, have you worked with this charity for at least a year?']]");
	public By byWorkedCharityNoInputField=By.name("workedCharityNoInputField");
	public By byWorkedCharityNoMessage=By.xpath("//*[text()='You must have worked with the charity for at least a year to qualify.']");

	public void setWorkedCharityNoInputField() {
		scrollUpDownToElement(webDriver, byWorkedCharityNoInputField, "If not, have you worked with this charity for at least a year? No", -200);
		selectRadioButton(byWorkedCharityNoInputField,"If not, have you worked with this charity for at least a year? No");
	}

	public By byWorkedCharityYesInputFieldInput=By.xpath("//input[@name='workedCharityYesInputField']");
	public By byWorkedCharityYesInputField=By.name("workedCharityYesInputField");
	public void setWorkedCharityYesInputField() {
		scrollUpDownToElement(webDriver, byWorkedCharityYesInputField, "If not, have you worked with this charity for at least a year? Yes", -200);
		selectRadioButton(byWorkedCharityYesInputField,"If not, have you worked with this charity for at least a year? Yes");
	}

	public void Verify_WorkedCharityYesInputFieldSelected(){
		if (isWebElementSelected(byWorkedCharityYesInputFieldInput,"WorkedCharityRadioYes", ExceptionHandling.FAIL)) 
			TestUtilities.logPassWithScreenshot("By default Yes is selected for: If not, have you worked with this charity for at least a year?");
		else
			TestUtilities.logFailedWithScreenshot("By default Yes is NOT selected for: If not, have you worked with this charity for at least a year?");
	}

	public void isNotWorkedCharityDivVisible() {
		if(isNotElementVisible(byWorkedCharityDiv,"If not, have you worked with this charity for at least a year?", ExceptionHandling.FAIL)) {
			TestUtilities.logPassWithScreenshot("[If not, have you worked with this charity for at least a year?] is not displayed");
		}
	}
	public void isWorkedCharityDivVisible() {
		if (isElementVisible(byWorkedCharityDiv,"If not, have you worked with this charity for at least a year?", ExceptionHandling.FAIL)) {
			TestUtilities.logPassWithScreenshot("[If not, have you worked with this charity for at least a year?] is displayed");
		}
	}

	public void isNotWorkedAtLeastaYearMessageVisible() {
		if(isNotElementVisible(byWorkedCharityNoMessage,"You must have worked with the charity for at least a year to qualify.", ExceptionHandling.FAIL)) {
			TestUtilities.logPassWithScreenshot("[You must have worked with the charity for at least a year to qualify.] is not displayed");
		}
	}
	public void isWorkedAtLeastaYearMessageVisible() {
		if(isElementVisible(byWorkedCharityNoMessage,"You must have worked with the charity for at least a year to qualify.", ExceptionHandling.FAIL)) {
			TestUtilities.logPassWithScreenshot("[You must have worked with the charity for at least a year to qualify.] is displayed");
		}
	}
	...
	public void ValidateCharityName() {
		...
		if("".equals(getCharityName())) 
			TestUtilities.logPassWithScreenshot("Charity Name is cleared after clicking ClearForm Button");
		else
			TestUtilities.logFailedWithScreenshot("Charity Name is NOT cleared after clicking ClearForm Button");
	}

	public void Verify_SelectingCharityDropDown_PopulatesCharityFields() {
		createNewMatchingRequest("MatchingRequest1","100");
		executeSQLUpdateAllMatchingRequestsStatus("12");
		refreshPage();
		setPreviousCharityRadioPositive();
		isNotWorkedCharityDivVisible();
		isCharityNameDivVisible();
		isCharityNameDisable();
		selectDropDownOption(byCharityDropDown, "RedCross", "Charity",ExceptionHandling.FAIL);
		scrollUpDownToElement(webDriver, byCharityName, "Charity Name", -200);
		if (verifyEquals("RedCross",getCharityName())) 
		...
		if (verifyEquals("support@redcross.com",getCharityContactEmail())) 
		...
		if (verifyEquals("+12-345-678-9012",getCharityPhone())) 
		...
		if (verifyEquals("www.redcross.com",getWebsite())) 
		...
		if (verifyEquals("Board member",getPosition())) 
		...
		if (verifyEquals("Assistance",getCharityPurpose())) 
		...
		if (verifyEquals("Mike Brown",getCharityContact())) 
		...
		if (verifyEquals("address1",getCharityAddress1())) 
		...
		if (verifyEquals("address2",getCharityAddress2())) 
		...
		if (verifyEquals("address3",getCharityAddress3())) 
		...
		if (verifyEquals("Newsletters",getPublicityOpportunities())) 
	...
	public void ValidateCharityDropDown() {
		...
		selectDropDownOption(byCharityDropDown, excel.getCellData("MatchingRequest1", "CharityName", 2), "Charity",ExceptionHandling.FAIL);
	...
	public void ValidateWebsite() {
		...
		if (isElementVisible(byWebsiteValidationMessage,websiteValidationMessage, ExceptionHandling.FAIL)) {
			...
			if (isNotElementVisible(byWebsiteValidationMessage,websiteValidationMessage, ExceptionHandling.FAIL)) {
				try {Thread.sleep(500);} catch (InterruptedException e) {e.printStackTrace();}
				TestUtilities.logPassWithScreenshot("Website validation message is cleared after clicking ClearForm Button");
			} else
				TestUtilities.logFailedWithScreenshot("Website validation message is NOT cleared after clicking ClearForm Button");
		} else
		...
		if (isElementVisible(byWebsiteValidationMessage,"Website should be in correct format", ExceptionHandling.FAIL)) { 
			...
			if(isNotElementVisible(byWebsiteValidationMessage,websiteValidationMessage, ExceptionHandling.FAIL)){
				...
				TestUtilities.logPassWithScreenshot("Website validation message is NOT cleared after clicking ClearForm Button");
		} else
		...
		if ("".equals(getWebsite())) 
			TestUtilities.logPassWithScreenshot("Website is cleared after clicking ClearForm Button");
		else
			TestUtilities.logFailedWithScreenshot("Website is NOT cleared after clicking ClearForm Button");
	}
	...
	public void ValidatePosition() {
		...
		if (isElementVisible(byPositionValidationMessage,positionValidationMessage, ExceptionHandling.FAIL)) {
			...
			if(isNotElementVisible(byPositionValidationMessage,positionValidationMessage, ExceptionHandling.FAIL)) {
				try {Thread.sleep(500);} catch (InterruptedException e) {e.printStackTrace();}
				TestUtilities.logPassWithScreenshot("Position validation message is cleared after clicking ClearForm Button");
			} else
				TestUtilities.logFailedWithScreenshot("Position validation message is NOT cleared after clicking ClearForm Button");
		} else
		...
		if ("".equals(getPosition())) 
			TestUtilities.logPassWithScreenshot("Position is cleared after clicking ClearForm Button");
		else
			TestUtilities.logFailedWithScreenshot("Position is NOT cleared after clicking ClearForm Button");
	}
	...
	public void ValidateCharityContact() {
		...
		if (isElementVisible(byCharityContactValidationMessage,charityContactValidationMessage, ExceptionHandling.FAIL)) {
			...
			if (isNotElementVisible(byCharityContactValidationMessage,charityContactValidationMessage, ExceptionHandling.FAIL)) {
				try {Thread.sleep(500);} catch (InterruptedException e) {e.printStackTrace();}
				TestUtilities.logPassWithScreenshot("Charity Contact validation message is cleared after clicking ClearForm Button");
			} else
				TestUtilities.logFailedWithScreenshot("Charity Contact validation message is NOT cleared after clicking ClearForm Button");
		} else
	...
	public void ValidateCharityAddress1() {
		...
		if (isElementVisible(byCharityAddress1ValidationMessage,charityAddress1ValidationMessage, ExceptionHandling.FAIL)) {
			...
			if(isNotElementVisible(byCharityAddress1ValidationMessage,charityAddress1ValidationMessage, ExceptionHandling.FAIL)) {
				try {Thread.sleep(500);} catch (InterruptedException e) {e.printStackTrace();}
				TestUtilities.logPassWithScreenshot("Charity Address 1 validation message is cleared after clicking ClearForm Button");
			} else
				TestUtilities.logFailedWithScreenshot("Charity Address 1 validation message is NOT cleared after clicking ClearForm Button");
		}
	...
	public void ValidateCharityAddress2() {
		...
		if ("".equals(getCharityAddress2())) 
			TestUtilities.logPassWithScreenshot("Charity Address2 is cleared after clicking ClearForm Button");
		else
			TestUtilities.logPassWithScreenshot("Charity Address2 is NOT cleared after clicking ClearForm Button");
	}
	...
	public void ValidateCharityAddress3() {
		...
		if (isElementVisible(byCharityAddress3ValidationMessage,charityAddress3ValidationMessage, ExceptionHandling.FAIL)) {
			...
			if(isNotElementVisible(byCharityAddress3ValidationMessage,charityAddress3ValidationMessage, ExceptionHandling.FAIL)) {
				try {Thread.sleep(500);} catch (InterruptedException e) {e.printStackTrace();}
				TestUtilities.logPassWithScreenshot("Charity Address 3 validation message is cleared after clicking ClearForm Button");
			} else
				TestUtilities.logFailedWithScreenshot("Charity Address 3 validation message is NOT cleared after clicking ClearForm Button");
		}
	...
	public void ValidateCharityPurpose() {
		...
		if (isElementVisible(byCharityPurposeValidationMessage,charityPurposeValidationMessage, ExceptionHandling.FAIL)) {
			...
			if(isNotElementVisible(byCharityPurposeValidationMessage,charityPurposeValidationMessage, ExceptionHandling.FAIL)) {
				try {Thread.sleep(500);} catch (InterruptedException e) {e.printStackTrace();}
				TestUtilities.logPassWithScreenshot("Charity Purpose validation message is cleared after clicking ClearForm Button");
			} else
				TestUtilities.logFailedWithScreenshot("Charity Purpose validation message is NOT cleared after clicking ClearForm Button");
		} else
	...
	public void createNewMatchingRequest(String sheetName, String amount, String... display) {
		...
		setDonationDate(excel.getCellData(sheetName, "DonationDate", 2));
		setPreviousCharityRadioNegative();
		setWorkedCharityYesInputField();
	...
	public void isAllCharityFieldsCleared() {
		...
		if (verifyEquals("",getCharityContact())) 
			TestUtilities.logPassWithScreenshot("CharityContact is cleared");
	...
	public void Verify_OutOfRangeValidation(String managementLevel, String managementLevelName, String amount) {
		...
		createNewMatchingRequest("MatchingRequest1",amount);
		executeSQLUpdateAllMatchingRequestsStatus("12");
		landingPage.clickCreateMatchingRequest();
		createNewMatchingRequest(amount,"DontShowToastMessage");
